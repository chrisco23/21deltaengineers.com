...
namespace DiviBooster\GalleryBooster\SliderHideArrows;
...
function add_field($fields) {
    ...
    $new_fields = array(
        'dbdb_show_arrows'    => array(
            'label'            => esc_html__('Show Slider Arrows', 'divi-gallery-booster'),
            'type'             => 'yes_no_button',
            ...
            'description'      => esc_html__('Choose whether or not the navigation arrows should be displayed on the gallery slider.', 'divi-gallery-booster'),
            'show_if' => array(
                'fullwidth' => 'on',
            )
        )
...
function enable_feature($output, $render_slug, $module) {
    ...
    if (empty($props['dbdb_show_arrows']) || $props['dbdb_show_arrows'] !== 'off') {
        return $output;
    }
...
function add_class($classes, $props) {
    ...
    $classes[] = 'dbdb-slider-arrows-off';
    return $classes;
}
...
function add_styles() { ?>
    <style>
        .et_pb_gallery.dbdb-slider-arrows-off .et-pb-slider-arrows {
