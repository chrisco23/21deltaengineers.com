...
function db_add_pre_get_portfolio_projects_random($props, $atts, $slug) {
    if (DBDB_module_slug::is_portfolio($slug)) {
        if (isset($atts['db_project_order']) && $atts['db_project_order'] === 'random') {
            add_action('pre_get_posts', 'db_randomize_portfolio_module_projects', 11); // After any custom pre_get_posts actions
        }
    }
    return $props;
}
...
function db_remove_pre_get_portfolio_projects_random($content) {
    remove_action('pre_get_posts', 'db_randomize_portfolio_module_projects', 11); // After any custom pre_get_posts actions
    return $content;
}
