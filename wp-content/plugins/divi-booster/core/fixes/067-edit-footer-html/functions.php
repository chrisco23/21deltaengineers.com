...
// [year] shortcode => 2019
function wtfdivi067_year_shortcode() {
    return '<span class="divibooster_year"></span><script>jQuery(function($){$(".divibooster_year").text(new Date().getFullYear());});</script>';
}
add_shortcode('year', 'wtfdivi067_year_shortcode');

// [yr] shortcode => 19
function wtfdivi067_yr_shortcode() {
    return '<span class="divibooster_yr"></span><script>jQuery(function($){$(".divibooster_yr").text(new Date().getFullYear().toString().substr(2,2));});</script>';
}
add_shortcode('yr', 'wtfdivi067_yr_shortcode');

// [copy] shortcode => &copy;
function wtfdivi067_copy_shortcode() {
    return '&copy;';
}
...
function db067_get_footer_credit() {
    ...
    $footerhtml = apply_filters('dbdb_option_value', $footerhtml, '067-edit-footer-html', 'footerhtml'); // allows wpml to translate
